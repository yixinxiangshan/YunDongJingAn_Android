...
public class TabPageIndicator extends HorizontalScrollView implements PageIndicator {
	 ...
	 */
	private int location = LOCATION_UP;

	/**
	 * Used to store the icon.
	 */
	private int[] drawables = new int[4];
	private Drawable[] drawableImgs = new Drawable[4];
	...
	private void addTab(int index, CharSequence text, int iconResId, ColorStateList colors) {
	...
	private void addTab(int index, CharSequence text, Drawable icon, ColorStateList colors) {
		final TabView tabView = new TabView(getContext());
		tabView.mIndex = index;
		tabView.setFocusable(true);
		tabView.setOnClickListener(mTabClickListener);
	...
	public void notifyDataSetChanged() {
		...
		for (int i = 0; i < count; i++) {
			...
			ColorStateList titleColor = iconAdapter.getTitleColorList(i);

			int iconResId = 0;
			Drawable iconResView = null;
			if (iconAdapter != null) {
				iconResId = iconAdapter.getIconResId(i);
				iconResView = iconAdapter.getIconDrawable(i);
			}
			if (iconResId != 0) {
				addTab(i, title, iconResId , titleColor);
			} else {
				addTab(i, title, iconResView ,titleColor);
			}
		}
